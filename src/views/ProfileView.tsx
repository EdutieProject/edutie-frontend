        ...
        <NavLayout activeSectionIdOverride={navSections.profile}>
            <Box sx={{display: "flex", justifyContent: "space-between"}}>
                ...
                <Box sx={{display: "flex", gap: theme.spacing(4), alignItems: "center"}}>
                    <RoundedButton label={"Uczeń"} active={currentView === SubView.STUDENT}
                                   onClick={() => setCurrentView(SubView.STUDENT)}/>
                    {/* <RoundedButton label={"Edukator"} active={currentView == Views.EDUCATOR} onClick={() => setCurrentView(Views.EDUCATOR)} /> */}
                    <RoundedButton label={"Ustawienia"} active={currentView === SubView.SETTINGS}
                                   onClick={() => setCurrentView(SubView.SETTINGS)}/>
                </Box>
            </Box>
            {
...
function StudentProfileView({setError}: { setError: Dispatch<SetStateAction<any>> }) {
    ...
    return (
        <Grid container flexGrow={1} spacing={theme.spacing(8)} marginTop={theme.spacing(6)}>
            <Grid item xs={6}
                  sx={{display: "flex", flexDirection: "column", alignItems: "stretch", gap: theme.spacing(1)}}>
                    ...
                    learningResults.length > 0 ?
                        learningResults.slice(0, 3).map((learningResult, i) => (
                            <Box sx={{
                                ...
                                <Divider flexItem orientation="horizontal" sx={{marginY: theme.spacing(2)}}/>
                                <Box sx={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
                                    <Box sx={{display: "flex", alignItems: "center", gap: theme.spacing(2)}}>
                                        {
                                            (learningResult.assessments as Array<any>).every((x) => x.grade > 4) ?
                                    ...
                                    <CircleButton sx={{justifySelf: "flex-end"}} size={"1rem"}
                                                  onClick={() => navigate(navigationPath.fillPath(navigationPath.learningResult, learningResult.id))}>
